...
1 - docker-compose -f app-python-docker-compose.yml build
2 - docker-compose -f app-python-docker-compose.yml up
...
1 - docker-compose -f db-mysql-docker-compose.yml build
2 - docker-compose -f db-mysql-docker-compose.yml up
3 - docker exec -it db-mysql /bin/bash
4 - mysql
5 - copiar (CTRL + C) o conteúdo do arquivo script_db_mysql.sql (anexado nesse repositório) e colar (CTRL + V) o conteúdo dentro do terminal do MySQL, e depois digitar ENTER
